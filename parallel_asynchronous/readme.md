...
> Процессам удобно обмениваться информацией через механизмы IPC (Interprocess Communication).

- Queues - аналогично потоковым очередям, позволяют передавать данные асинхронно.
- Pipes - создают двустороннюю связь между двумя процессами.
- Shared Memory - использование общей памяти для быстрого обмена большими объемами данных.
- Locks/Semaphores - синхронизация процессов, предотвращение конфликта доступа к ресурсам.
